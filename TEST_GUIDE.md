...
### 시나리오 2: 교통/숙박 다일자 지출 테스트

#### 2-1. 숙박/교통 지출 추가
1. 대시보드에서 "지출 추가" 클릭
2. 카테고리에서 `숙박` 또는 `교통` 선택
3. 금액 입력 (예: 300000)
4. 시작일/종료일 선택 (예: 3일간)
5. 전체 참여자를 선택한 뒤 날짜별 참여자를 조정
6. "추가하기" 클릭
7. ✅ 대시보드 날짜별 지출 목록에 표시되는지 확인

#### 2-2. 날짜별 참여자 정산 확인
1. 대시보드로 돌아가기
2. ✅ 해당 지출 상세에서 날짜별 참여자 계산식이 표시되는지 확인
3. ✅ 개인별 정산 현황에 날짜별 참여자만 부담자로 반영되는지 확인
