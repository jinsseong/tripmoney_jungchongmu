...
interface ExpenseFormPreferences {
  payerId?: string;
...
const getPreferenceKey = (tripId: string) =>
  "jungchongmu-expense-preferences:" + tripId;
...
export const ExpenseForm: React.FC<ExpenseFormProps> = ({
  ...
  const [paymentType, setPaymentType] = useState<"cash" | "card">(
    initialExpense?.payment_type || "card"
  );
  ...
  const hydratedPreferenceTripId = useRef("");
  ...
  useEffect(() => {
    if (
      ...
      categories.length === 0 ||
      hydratedPreferenceTripId.current === trip.id
    ) {
      return;
    }
    ...
    try {
      const rawPreferences = localStorage.getItem(getPreferenceKey(trip.id));
      if (!rawPreferences) return;

      const preferences = JSON.parse(rawPreferences) as ExpenseFormPreferences;
      ...
      if (preferences.payerId && participantIdSet.has(preferences.payerId)) {
        setPayerId(preferences.payerId);
      }
      if (preferences.paymentType) {
        setPaymentType(preferences.paymentType);
      }
      if (preferences.currency) {
        setCurrency(preferences.currency);
      }
      if (preferences.categoryId && categoryIdSet.has(preferences.categoryId)) {
        setCategoryId(preferences.categoryId);
      }
      if (preferences.participantIds) {
        setSelectedParticipantIds(
          preferences.participantIds.filter((participantId) =>
            participantIdSet.has(participantId)
          )
        );
      }
    } catch {
      // 저장된 입력 기본값이 손상되어도 새 지출 입력은 계속 사용할 수 있습니다.
    }
  }, [categories, isEditMode, participants, trip]);
  ...
  useEffect(() => {
    if (isMultiDayCategory && endDate) {
      ...
      setDailyParticipants((previous) => {
        const next: Record<string, string[]> = {};
        dates.forEach((dateString) => {
          next[dateString] = (
            previous[dateString] || selectedParticipantIds
  ...
  }, [isMultiDayCategory, date, endDate, selectedParticipantIds]);
  ...
  const submitExpense = async (extendTripDates = false) => {
    ...
    try {
      ...
      if (!isEditMode && trip) {
        const preferences: ExpenseFormPreferences = {
          payerId,
          paymentType,
          currency,
        ...
        try {
          localStorage.setItem(
            getPreferenceKey(trip.id),
            JSON.stringify(preferences)
          );
        } catch {
          // localStorage를 사용할 수 없는 환경에서도 저장 완료 처리는 유지합니다.
        }
      }
  ...
  const applyParticipantSelection = (
    nextParticipantIds: string[],
    syncEveryDate = false
  ) => {
    setFormError("");
    setSelectedParticipantIds(nextParticipantIds);
    setCustomAmounts((previous) =>
      Object.fromEntries(
        Object.entries(previous).filter(([participantId]) =>
    ...
    if (syncEveryDate) {
      setDailyParticipants((previous) =>
        Object.fromEntries(
          Object.keys(previous).map((dateString) => [
            dateString,
            [...nextParticipantIds],
          ])
        )
    ...
    <form onSubmit={handleSubmit} className="space-y-6 pb-1">
      ...
      <div>
        ...
        <div className="relative">
          <DollarSign className="absolute left-3.5 top-1/2 h-5 w-5 -translate-y-1/2 text-[var(--muted-2)]" />
          <input
            type="text"
            inputMode="numeric"
            enterKeyHint="next"
            ...
            placeholder="0"
            className="min-h-[60px] w-full rounded-lg border border-[var(--line-strong)] bg-[var(--surface)] px-12 text-center text-2xl font-extrabold text-[var(--foreground)] focus:border-[var(--primary)] focus:outline-none focus:ring-3 focus:ring-[var(--primary)]/15"
            required
          />
        </div>
        ...
        <div className="mt-2 grid grid-cols-2 gap-2">
          <select
            ...
            className="min-h-[48px] min-w-0 rounded-lg border border-[var(--line-strong)] bg-[var(--surface)] px-3.5 text-base text-[var(--foreground)] focus:border-[var(--primary)] focus:outline-none focus:ring-3 focus:ring-[var(--primary)]/15"
          >
      ...
      <div className="space-y-2">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            ...
            <input
              ...
              className={cn(
                "min-h-[48px] w-full rounded-lg border px-3.5 text-base focus:border-[var(--primary)] focus:outline-none focus:ring-3 focus:ring-[var(--primary)]/15",
                trip && !isDateWithinTripRange(date) && !isMultiDayCategory
                  ? "border-[var(--warning)] bg-[var(--surface-warning)]"
        ...
        {trip && !getDateRangeValidation().isValid && (
          <div className="rounded-lg border border-[var(--surface-warning)] bg-[var(--surface-warning)] p-3">
            <p className="text-sm leading-6 text-[var(--warning-ink)]">
      ...
      <div>
        ...
        <select
          ...
          className="min-h-[48px] w-full rounded-lg border border-[var(--line-strong)] bg-[var(--surface)] px-3.5 text-base text-[var(--foreground)] focus:border-[var(--primary)] focus:outline-none focus:ring-3 focus:ring-[var(--primary)]/15"
          required
        >
          ...
          onToggle={(id) => {
            applyParticipantSelection(
              selectedParticipantIds.includes(id)
                ? selectedParticipantIds.filter((participantId) => participantId !== id)
                : [...selectedParticipantIds, id]
            );
          }}
          onSelectionChange={applyParticipantSelection}
        />
      ) : (
        <>
          {/* 전체 참여자 선택 (날짜별 기본값) */}
          <div>
            ...
            <ParticipantSelector
              ...
              onSelectionChange={(participantIds) =>
                applyParticipantSelection(participantIds, true)
              }
            />
          </div>

          {/* 날짜별 참여자 선택 */}
          <div className="space-y-4 rounded-lg border border-[var(--line)] bg-[var(--surface-muted)] p-3 sm:p-4">
            <h3 className="text-sm font-bold text-[var(--muted-strong)]">
              날짜별 참여자 선택
            </h3>
          ...
          {selectedParticipantIds.map((pid) => {
            ...
            return (
              <div key={pid} className="grid grid-cols-1 gap-2 sm:grid-cols-[5rem_1fr] sm:items-center">
                <span className="text-sm font-bold text-[var(--muted-strong)] sm:w-20">
                  {participant?.name}
                </span>
                <input
                  type="text"
                  ...
                  placeholder="0"
                  className="min-h-[48px] min-w-0 rounded-lg border border-[var(--line-strong)] px-3.5 text-base focus:border-[var(--primary)] focus:outline-none focus:ring-3 focus:ring-[var(--primary)]/15"
                />
              </div>
            );
          })}
          <div
            className={cn(
              ...
              hasMissingCustomAmount || hasCustomAmountMismatch
                ? "border-[var(--surface-danger)] bg-[var(--surface-danger)] text-[var(--danger-ink)]"
                : "border-[var(--line-success)] bg-[var(--surface-success)] text-[var(--success-ink)]"
            )}
          >
      ...
      {settlementType === "equal" &&
        ...
        selectedParticipantIds.length > 0 && (
          <div className="rounded-lg border border-[var(--surface-selected)] bg-[var(--primary-soft)] p-4">
            <p className="mb-1 text-sm font-bold text-[var(--muted-strong)]">정산 미리보기</p>
            <p className="break-words text-base font-extrabold text-[var(--primary-pressed)] sm:text-lg">
      ...
      {visibleValidationMessage && (
        <div className="rounded-lg border border-[var(--surface-danger)] bg-[var(--surface-danger)] p-3 text-sm font-bold text-[var(--danger-ink)]">
          {visibleValidationMessage}
        </div>
      )}

      {/* 버튼 */}
      <div className="sticky bottom-0 z-10 -mx-4 flex flex-col-reverse gap-2 border-t border-[var(--line)] bg-[rgb(var(--surface-rgb)/95%)] px-4 py-3 backdrop-blur sm:static sm:mx-0 sm:flex-row sm:border-t-0 sm:bg-transparent sm:px-0 sm:py-0 sm:backdrop-blur-none">
        {onCancel && (
          <Button
            type="button"
            variant="outline"
      ...
      >
        <div className="space-y-4">
          <div className="rounded-lg border border-[var(--surface-warning)] bg-[var(--surface-warning)] p-4">
            <p className="mb-2 text-sm font-bold text-[var(--warning-ink)]">
